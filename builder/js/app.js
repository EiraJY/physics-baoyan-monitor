(function(){
  ...
  function structuredMaterials(items){
    return (Array.isArray(items)?items:[]).filter(x=>x&&String(x.title||'').trim()).map(x=>material(String(x.title).trim(),Number(x.level)||1,x.required!==false));
  }
  function mergeLatestMaterials(project,items){
    const incoming=structuredMaterials(items);if(!incoming.length)return false;
    const oldByTitle=new Map((project.materials||[]).map(m=>[String(m.title||'').trim(),m]));
    project.materials=incoming.map(m=>{const old=oldByTitle.get(m.title);return old?{...m,id:old.id,files:old.files||[],include:old.include!==false}:m;});
    return true;
  }
  ...
  async function importNoticeFromQuery(){
    ...
    if(noticeId){
      try{
        ...
        if(x)seed={sourceNoticeId:x.id,name:`${x.school}${x.college||''} ${x.route}申请材料`,school:x.school,college:x.college,route:x.route,year:d.meta?.publish_year||currentYear(),publishedAt:params.get('publishedAt')||x.published_at,deadline:params.get('deadline')||x.deadline_display||x.deadline,sourceUrl:x.url,noticeMaterials:x.materials_text||x.materials||x.excerpt||'',structuredMaterials:x.material_items||[],profile:clone(profileDefault)};
      }catch(err){console.warn('通知数据读取失败',err);}
    }else if(params.get('school'))seed={sourceNoticeId:params.get('sourceId')||uid('static'),name:`${params.get('school')||''}${params.get('college')||''} ${params.get('route')||'预推免'}申请材料`,school:params.get('school')||'',college:params.get('college')||'',route:params.get('route')||'预推免',year:Number(params.get('year'))||currentYear(),publishedAt:params.get('publishedAt')||'',deadline:params.get('deadline')||'',sourceUrl:params.get('sourceUrl')||'',noticeMaterials:params.get('materials')||'',structuredMaterials:[],profile:clone(profileDefault)};
    if(!seed)return;
    let p=state.projects.find(x=>x.sourceNoticeId===seed.sourceNoticeId);
    if(p){
      Object.assign(p,{name:seed.name,school:seed.school,college:seed.college,route:seed.route,year:seed.year,publishedAt:seed.publishedAt,deadline:seed.deadline,sourceUrl:seed.sourceUrl,noticeMaterials:seed.noticeMaterials,noticeImported:true});
      mergeLatestMaterials(p,seed.structuredMaterials);state.activeId=p.id;
    }else{
      const initial=structuredMaterials(seed.structuredMaterials);
      p=newProject({...seed,materials:initial.length?initial:defaultMaterials()});state.projects.push(p);state.activeId=p.id;
    }
